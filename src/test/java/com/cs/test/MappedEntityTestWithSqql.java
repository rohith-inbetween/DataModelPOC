...
@ContextConfiguration(locations = { "file:src/main/resources/SqlMappedContext.xml" })
@Transactional
@TransactionConfiguration(defaultRollback = true)
public class MappedEntityTestWithSqql {
	...
	public void createEntity() {
		...
		System.out.println("!@#$!@$#!@$!@$"
				+ entityRepository.findOne(product.getId()).getId());
	}
	...
	public void createAndGetEntitiesWithMapppingAndValues() {
		...
		EntityDTO productFromDB = (EntityDTO) entityRepository
				.getMapppingForGivenEntity(product.getId());

		System.out.println("******Created And Retrieved Mapped Entity \n"
				+ new Gson().toJson(productFromDB));
		Assert.assertEquals(attributeValues.get(sizeAttribute.getId() + ""),
				productFromDB.getAttributeValues().get(sizeAttribute.getName()));
		Assert.assertEquals(attributeValues.get(weightAttribute.getId() + ""),
				productFromDB.getAttributeValues().get(lengthAttribute.getName()));
		Assert.assertEquals(attributeValues.get(weightAttribute.getId() + ""),
				productFromDB.getAttributeValues().get(lengthAttribute.getName()));
		Assert.assertEquals("TestClass1",
				productFromDB.getClasses().get(myClass.getId() + "").getName());
		Assert.assertEquals("TestClass2",
				productFromDB.getClasses().get(myClass1.getId() + "").getName());
	}
	...
	public void searchForValues() {
		...
		attributeValues.put(weightAttribute.getId() + "", "500kg");
		attributeValues.put(lengthAttribute.getId() + "", "500m");
		product.setUpdatedClassAttributeValues(attributeValues);
		entityRepository.update(product);
		...
		product2.setUpdatedClassAttributeValues(attributeValues2);
	...
	public void checkUpdateOfAttribute() {
		...
		AttributeDTO attrFromDB = (AttributeDTO)attrRep.findOne(weightAttribute.getId());
		System.out.println("@#%#%#%@Updated Attribute\n"
				+ new Gson().toJson(attrFromDB));
		Assert.assertEquals(attrFromDB.getName(), "anindyaUpdated");
	}
	...
	public void checkDeleteOfEntity() {
		...
		System.out.println(entityRepository.findOne(20l));
	}
