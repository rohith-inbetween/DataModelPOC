...
public class MySQLMappedClazzStrategy implements IClazzStrategy {
	...
	public ClassDTO save(ClassDTO bObject) {
		...
		ClazzMapped c = (ClazzMapped) ObjectConverter.convertToMappedModel(bObject);
		ArrayList<Attribute> attributes = new ArrayList<Attribute>();
		List<String> attributeIds = bObject.getAddedAttributes();	
		if (attributeIds != null && attributeIds.size() > 0) {
			for (String attributeId : attributeIds) {
				Attribute attribute = (Attribute)connector.getById(Long.valueOf(attributeId), Attribute.class);
				c.addAttribute(attribute);
			}
		}
		c = (ClazzMapped) connector.save(c);
